pipeline {
    ...
    stages {
        stage("Clone API Discovery Server from Github") {
            steps {
                echo "Cloning API Discovery Server from Github. Please Wait..."
                git branch: 'main',
                    url: 'https://github.com/Elleined/api-discovery-server'
                echo "Cloning API Discovery Server from Github. Success!"
            }
        }
        ...
        stage("Create Docker Image") {
            steps {
                ...
                sh 'docker build -t api-discovery-server:latest .'
        ...
        stage("Push docker image to DockerHub") {
            steps {
                ...
                sh 'docker tag api-discovery-server:latest elleined/api-discovery-server:latest'
                sh 'docker push elleined/api-discovery-server:latest'
